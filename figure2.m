...
vertices = [1641 + 33i, 1433 + 737i, 1999 + 1195i, 1269 + 1195i, 1012 + 1881i, 770 + 1195i, 22 + 1195i, 605 + 737i, 398 + 33i,1019 + 447i];
p = polygon(vertices);
p
...
y_vals = linspace(0.00001, 1, 10);   % Horizontal lines (streamlines)
x_vals = linspace(-1, 1, 200);
x_vals_eq = linspace(-1, 1, 10);  % Vertical lines (equipotential)
y_vals_eq = linspace(0.00001, 1, 200);
...
for y = y_vals
    z_line = x_vals + 1i*y;
    h_stream = [h_stream, plot(real(z_line), imag(z_line), 'r')];
end
...
title('Original Grid in Upper Half-Plane');
xlabel('Re(z)'); ylabel('Im(z)');
axis equal tight; grid on;
% legend([h_pre], {'Prevertices'}, ...
...
title('Mapped Grid in Star Domain');
